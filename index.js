...
/* Task 5: Create a function called get20s() that takes data as an argument and returns an array with names of artists who were born the 20th 
century (1800-1900) */
...
function get20s(/* Code here */){
  for(let i=0; i<artists.length; i++){  
    if (artists[i].years.substr(0,2) ==='18'){
        bornArr.push(artists[i].name);
    }
  };
  return (bornArr);  
...
/* Task 6: Create a function called lotsOfArt() that takes artists as an argument and returns an array with names of artists who painted more 
than 100 paintings */
...
/* Task 7: Create a function called `addArtist` that can accept an array of information and add it to the artists array. Then, Add a 21st 
artist to the array (you) with custom information! 👩‍🎨👨‍🎨
...
console.log();
console.log("Task 71:");
function addArtist(artist21){
 artists.push(artist21);
 console.log(artists[artists.length-1]); 
  }
addArtist({id: 21,
  name: "Tanya Efimiva", 
  years: "19XX - current day",
  genre: "Web Design", 
  nationality: "Russian",
  bio: "Tanya worked as teacher of math and compuper sience, also she was Sysadmin, programmer on Delphi and Admin DB"
});
...
function checkArtist(checkName){
for(let i=0; i<artists.length; i++){
  if (checkName === artists[i].name){
    console.log(checkName,'is in the dataset');
  }
 }
}
checkArtist('El Greco');
